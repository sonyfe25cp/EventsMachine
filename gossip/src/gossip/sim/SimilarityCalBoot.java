...
public class SimilarityCalBoot extends Boot {
	...
	public void process() {
		if(date == 0){ //全部计算
			...
			for(int begin = 20130101; begin <= today; begin = DateTrans.theDayAfterYYMMDD(begin)){
				process_once(begin);
			}
		}else{
			process_once(date);
	...
	public void process_once(int date) {
		try {
			...
			for (String field : fields) {
				List<SimilarDocPair> sdpList = calculator.getSimilarDocPairs(date, 15, field);// 后退15天求相似度
				if (sdpList == null)
					continue;
	...
	public static void main(String[] args) {
		...
		SimilarityCalBoot scb = new SimilarityCalBoot(20130110);
		scb.run();
	}
