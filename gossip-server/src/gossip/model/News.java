...
public class News {
	...
	public Document toDocument() {
		...
		Field uniqueField = new Field("id", id + "", Store.YES,
				Index.NOT_ANALYZED);
		Field titleField = new Field("title", title, Store.YES, Index.ANALYZED,
				TermVector.YES);
		Field contentField = new Field("body", body, Store.YES, Index.ANALYZED,
				TermVector.YES);
		Field authorField = new Field("author", author == null ? "" : author,
		...
		Field siteField = new Field("site", fromSite == null ? "" : fromSite,
				Store.YES, Index.ANALYZED);
		NumericField crawlAtField = new NumericField("crawlat",
				Field.Store.YES, true).setIntValue(0);// 便于范围查找
		crawlAtField.setIntValue(Integer.parseInt(crawlAt));
		Field statusField = new Field("status", status, Store.NO,
				Index.NOT_ANALYZED);
		doc.add(uniqueField);
		doc.add(titleField);
